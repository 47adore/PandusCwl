-- Case Paradise | TURCJIA HUB v13 | XENO SAFE PROFESSIONAL EDITION
-- Fixed CoreGui + Xeno Compatible + Anti-Crash System
...
local COLORS = {
    ...
    WARNING = Color3.fromRGB(270, 220, 90), INFO = Color3.fromRGB(120, 180, 270),
...
-- ========== XENO SAFE GUI CREATOR ==========
local function createXenoSafeGUI()
    -- XENO SAFE: Use PlayerGui instead of CoreGui
    ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "TurcjaHubV13_XenoSafe"
    ...
    MainFrame.Name = "MainFrameXenoSafe"
    ...
    titleLabel.Text = "🔥 TURCJIA HUB v13 | XENO SAFE | CASE PARADISE 🔥"
    ...
    statusLabel.Text = "✅ XENO COMPATIBLE | Press X | 50+ Features Active"
    ...
    tabContainer.Name = "TabContainerXeno"
    ...
    contentArea.Name = "ContentAreaXeno"
    ...
    for i, tabName in ipairs(tabNames) do
        ...
        tabBtn.MouseButton1Click:Connect(function()
            for j, frame in pairs(tabFrames) do 
                frame.Visible = false
                local btn = tabContainer:FindFirstChild("TabXeno" .. j)
            ...
            TweenService:Create(tabBtn, TweenInfo.new(0.4, Enum.EasingStyle.Back), {
                Size = UDim2.new(0.16, -10, 1, -30), BackgroundTransparency = 0
            }):Play()
        end)
    end
    ...
    populateXenoSafeContent(tabFrames)
end

-- ========== XENO SAFE TOGGLE ==========
function createXenoToggle(parent, text, callback)
...
-- ========== XENO SAFE BUTTON ==========
function createXenoButton(parent, text, callback)
...
-- ========== POPULATE TABS ==========
function populateXenoSafeContent(tabFrames)
    -- AUTOFARM TAB
    createXenoToggle(tabFrames[1], "🚀 Auto Open Money Cases", toggleAutoFarm)
    createXenoToggle(tabFrames[1], "💰 Auto Sell Everything", toggleAutoSell)
    createXenoButton(tabFrames[1], "⚡ Instant Collect x25", function()
        for i = 1, 25 do safeFireRemote("Collect") safeFireRemote("Claim") end
    end)
    
    -- MOVEMENT TAB
    createXenoToggle(tabFrames[2], "✈️ Ultimate Fly (WASD)", toggleFly)
    createXenoToggle(tabFrames[2], "⚡ Speed x8 (120)", function(state)
        local char = player.Character
        if char and char:FindFirstChildOfClass("Humanoid") then
            char:FindFirstChildOfClass("Humanoid").WalkSpeed = state and 120 or 16
        end
    end)
    createXenoToggle(tabFrames[2], "🦘 Super Jump x5", function(state)
        local char = player.Character
        if char and char:FindFirstChildOfClass("Humanoid") then
    ...
    -- EVENTS TAB
    createXenoButton(tabFrames[3], "🎁 Teleport Gifts", function()
        local char = player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = CFrame.new(0, 60, 0)
        end
    end)
    createXenoButton(tabFrames[3], "📦 Teleport Cases", function()
        local char = player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
    ...
    -- MISC TAB
    createXenoToggle(tabFrames[4], "💡 Fullbright", function(state)
        Lighting.Brightness = state and 6 or 1
        Lighting.GlobalShadows = not state
    end)
    createXenoToggle(tabFrames[4], "🛡️ Anti AFK", function(state)
        task.spawn(function()
            while state do
    ...
    -- TROLL TAB
    createXenoButton(tabFrames[5], "💥 FLING ALL PLAYERS", flingAll)
    createXenoButton(tabFrames[5], "🌪️ RANDOM TP TORNADO", function()
        for _, plr in pairs(Players:GetPlayers()) do
            if plr ~= player and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
    ...
    -- PLAYER TAB
    createXenoToggle(tabFrames[6], "👁️ ULTIMATE ESP", toggleESP)
end

-- ========== XENO SAFE START ==========
task.spawn(function()
    task.wait(2)
    pcall(createXenoSafeGUI)
    
    -- XENO SAFE NOTIFICATION
    pcall(function()
        game.StarterGui:SetCore("SendNotification", {
            Title = "🔥 TURCJIA HUB v13 XENO SAFE";
            Text = "✅ LOADED PERFECTLY | Press X | Case Paradise Ready!";
            Duration = 8;
        })
    end)
    
    print("✅ TURCJIA HUB v13 XENO SAFE - LOADED!")
    print("🎮 Toggle: X | All features optimized for Xeno!")
end)
